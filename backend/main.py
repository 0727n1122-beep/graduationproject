...
★ CODE_DUMP (코드 전체 전송) — 비개발자 토큰 낭비의 최대 원인
  ...
  [replacement 생성 규칙] 코드 본문은 절대 삭제·요약·축약하지 않는다. verbatim 그대로
  replacement 안에 포함시킨다 — 다음 턴에서 실제로 코드를 고치려면 원본이 반드시 남아
  있어야 하기 때문이다. 축약 대상은 코드가 아니라 코드를 둘러싼 장황한 텍스트다
  (에러 스택 트레이스 원문, 재현용으로 붙인 중복 호출부 등).
...
5. 코드 덤프는 코드 본문을 절대 삭제·요약하지 말 것. 진단 문구만 코드 위에 덧붙이고
   코드는 verbatim 그대로 보존할 것 (원본이 사라지면 다음 턴에서 그 코드를 고칠 수 없음).
