...
★ MONOLITHIC_REQUEST (분할 없는 통합 요청) — 비개발자 특유의 패턴
  ...
  [핵심 원칙] 기능 개수는 사용자가 요청한 것 기준으로 센다. steps를 몇 단계로
  나눌지는 이 판단과 완전히 별개의 나중 결정이다. "회원가입 하나"를 요청받았으면,
  그걸 구현하는 코드를 2단계로 짜든 5단계로 짜든 요청은 여전히 1개다. steps를 잘게
  쪼갤수록 마치 기능이 여러 개인 것처럼 보이는 건 착시다 — steps 개수가 3개 이상이라고
  발동 조건을 만족하는 게 절대 아니다. 발동 여부를 먼저 정하고, 그다음에 steps를
  만들 것. steps를 먼저 구상한 뒤 그 개수로 발동 여부를 역산하지 말 것.
